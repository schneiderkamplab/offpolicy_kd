...
export JOBID=dyna_commonpile_continual
export TORCH_COMPILE=0
export PYTORCH_CUDA_ALLOC_CONF="expandable_segments:True" 
export OMP_NUM_THREADS=48
export TORCHDYNAMO_VERBOSE=1
mkdir -p logs/continual/$JOBID
...
accelerate launch \
   ...
   --mixed_precision bf16 \
    ...
    --overwrite \
    --yes \
    --save-path checkpoints/continual \
    --patience 1000 \
    > >(tee logs/continual/$JOBID/stdout.txt) \
    2> >(tee logs/continual/$JOBID/stderr.txt >&2)
