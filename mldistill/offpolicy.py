...
def distill(
    ...
    with timing(times, key="timing/prepare_dataloaders"):
        ...
        if distribution is not None and all(row[0] == 0 for row in distribution): # in this case, we are doing on policy distillation (if the first num<1, the other % of that comes from on-policy), so we need to adjust the max_seq_length, we'll need to adjust this when we do offpolicy in one epoch and then switch to on-policy
